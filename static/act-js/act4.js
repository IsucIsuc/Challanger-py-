...
async function sendData(seconds) {
  try {
      const response = await fetch('http://127.0.0.1:5000/won/4', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json'
          },
          body: JSON.stringify({ number: seconds, lastlvl: 4 })
      });
...
function timeout(){
  ...
  if(i > 49){
    ...
    document.getElementById(`button`).setAttribute(`href`, `/won/4`)
