...
async function sendData(seconds) {
  try {
      const response = await fetch('http://127.0.0.1:5000/won/16', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json'
          },
          body: JSON.stringify({ number: seconds, lastlvl: 16 })
      });
...
function timeout(){
  ...
  if(i > 99){
    ...
    document.getElementById(`button`).setAttribute(`href`, `/won/16`)
